...
def get_address(country=None):
    ...
    for i in range(5, 12):
        ...
        for addr in addrFile:
            try:
                if country is None or (len(addr) > 12 and addr[12] == country):
                    if len(addr) > i and addr[i] != '':
                        allAddrs.append(addr[i])
            except:
                pass
        full_addr.append(choice(allAddrs) if allAddrs else ("N/A" if i != 11 else (country or "N/A")))
...
class Person:
    def __init__(self):
        ...
        self.address = get_address(self.country)
        self.customAttr = {}
